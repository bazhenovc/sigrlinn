...
static void dxProcessDrawQueue(internal::DrawQueue* queue)
{
	...
	for (internal::DrawCall& call: queue->getDrawCalls()) {
		...
		for (size_t i = 0; i < 8; ++i) {
			ID3D11Buffer* constantBuffer = static_cast<ID3D11Buffer*>(call.constantBuffers[i].value);
			g_pImmediateContext->VSSetConstantBuffers(i, 1, &constantBuffer);
			g_pImmediateContext->HSSetConstantBuffers(i, 1, &constantBuffer);
			g_pImmediateContext->DSSetConstantBuffers(i, 1, &constantBuffer);
			g_pImmediateContext->GSSetConstantBuffers(i, 1, &constantBuffer);
			g_pImmediateContext->PSSetConstantBuffers(i, 1, &constantBuffer);
		}
...
void setConstantBuffer(DrawQueueHandle handle, uint32_t idx, BufferHandle buffer)
{
	if (handle != DrawQueueHandle::invalidHandle()) {
		internal::DrawQueue* queue = static_cast<internal::DrawQueue*>(handle.value);
